...
struct Context
{
	...
	static int32 renderThread(bx::Thread* /*_self*/, void* /*_userData*/)
	{
	...
	BGFX_API_FUNC(OcclusionQueryResult::Enum getResult(OcclusionQueryHandle _handle, int32* _result))
	{
	...
	BGFX_API_FUNC(uint32_t submit(uint8_t _id, ProgramHandle _program, OcclusionQueryHandle _occlusionQuery, int32 _depth, bool _preserveState))
	{
	...
	BGFX_API_FUNC(uint32_t submit(uint8_t _id, ProgramHandle _handle, IndirectBufferHandle _indirectHandle, uint16_t _start, uint16_t _num, int32 _depth, bool _preserveState))
	{
	...
	// render thread
	void flip();
	RenderFrame::Enum renderFrame(int32 _msecs = -1);
	...
	bool apiSemWait(int32 _msecs = -1)
	{
		if (m_singleThreaded)
		{
			return true;
